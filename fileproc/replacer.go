...
import (
	...
	"github.com/picfight/pfcd_builder/settings"
	"os"
	"path/filepath"
	"strings"
)

func TransferFiles(detector fileproc.FileToProcessDetector, from string, to string) {

	inputFiles := projectops.ListInputProjectFiles(from)

	for _, f := range inputFiles {
		postfix := strings.TrimPrefix(f, from)
		postfix = set.FileNameProcessor(postfix, detector)
