...
func (process *ExternalProcess) Wait() {
...
func (process *ExternalProcess) Launch(debugOutput bool) {
	if process.isRunning {
		lang.ReportErr("Process is already running: %v", process.runningCommand)
	}
	...
	pin.D("run", process.FullConsoleCommand())
...
func (process *ExternalProcess) Stop() error {
	if !process.isRunning {
		lang.ReportErr("Process is not running: %v", process.runningCommand)
	}
